...
%location of data
fpath = '/Users/jonathankirsh/Documents/DrumCircle/Aug15Pilot/04_all_quartet/';
fextension = '.experiment';

%% Experiment Mac 3 (08/12/25)
if 1
    snames = {'DCP21','DCP22','DCP23','DCP24','DCP25','DCP26'};
    exppath = '/Users/jonathankirsh/Documents/DrumCircle/Aug15Pilot/';
    fnames = {'04a_all_all_nodelay_quartet', '04b_all_all_120msdelay_quartet', '04c_all_all_240msdelay_quartet'};
    nTappers = 6;
    fnames = fnames(1:3)
end
...
outdir = fullfile(fpath, 'exports_R');
if ~exist(outdir, 'dir'), mkdir(outdir); end

combined_csv = fullfile(outdir, 'all_trials_long.csv')
...
for fi = ftodo
    ...
    %     ];
    %colors = parula(6);
    colors_all = [0.9*linspace(1,0,6)', zeros(6,1), 0.9*linspace(0,1,6)'];  % Blue → Red
    ...
    groups = {
        [1, 2, 3, 4],  ... Group 1: define channels
        [5, 6]   % Group 2: define channels
    };
    
    % Set the group index
    iG = 2;  % Change this to switch between groups
    ...
    for k = 1:numChannels
        ...
        if ~isempty(iChan)
            ...
            plot(xtap{k}, ytap, '.','MarkerEdgeColor',colors_local(k,:), 'MarkerFaceColor', colors_local(k,:), 'markersize',10)
    ...
    for k = 1:numChannels
        chan = chans(k);
        nRows = numel(ITI{k});
        T = [T; table(repmat(chan, nRows, 1), ITI{k}, times{k}, velocities{k}, ...
            'VariableNames', {'Drummer', 'ITI', 'Time', 'Velocity'})];
    end

    writetable(T, fullfile(fpath, [fnames{fi} '_ITI.csv']));

    % save figure
    if 0
        expmulti(gcf,'pdf',fnames{fi}, 600)
        close
    end
